...
Shader::Shader(const char* name, const Defines& defines) {
	load(name, defines);
}
...
void Shader::load(const char* name, const Defines& defines) {
	Parser* parser = new Parser(name, defines);
	// matrixes
	num_matrixes = 0;
...
void Shader::bind() 
{
	...
	for (int i = 0; i < num_matrixes; i++) {
		...
		else if (matrixes[i].type == LIGHT_TRANSFORM && Engine::lights.current_light) glLoadMatrixf(Engine::lights.current_light->transform);
	}
