...
Parser::Parser(const char* name, const Defines& defines) : data(NULL) {
	...
	while (*s) {
		...
		if (*s == '#' && !strncmp(s + 1, "ifdef", 5)) {
			...
			sscanf(s + 6, "%s", define);
			while (*s != '\0' && *s != '\n') s++;
			if (defines.isDefine(define) == 1) define_ok[depth++] = 1;
			...
			sscanf(s + 7, "%s", define);
			while (*s != '\0' && *s != '\n') s++;
			if (defines.isDefine(define) == 0) define_ok[depth++] = 1;
			else define_ok[depth++] = 0;
		}
