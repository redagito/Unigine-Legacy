...
void Map::load(const char* name, Paths& paths) {
	...
	try {
		while (*data) {
			...
			else if (!strcmp(token, "bsp")) load_bsp(paths);
			else if (!strcmp(token, "light")) load_light(paths);
			else if (!strcmp(token, "fog")) load_fog(paths);
			else if (!strcmp(token, "mirror")) load_mirror(paths);
			else if (!strcmp(token, "mesh")) load_mesh(paths);
			else if (!strcmp(token, "particles")) load_particles(paths);
...
void Map::load_bsp(const Paths& paths) {
	read_token("{");
	Engine::bsp = new Bsp();
	while (1) {
		const char* token = read_token();
		if (!token || !strcmp(token, "}")) break;
		else if (!strcmp(token, "mesh")) Engine::bsp->load(paths.findFile(read_string()));
		else if (!strcmp(token, "save")) Engine::bsp->save(read_string());
		else if (!strcmp(token, "material")) {
			const char* name = read_string();
			Engine::bsp->bindMaterial(name, Engine::loadMaterial(read_string(), paths));
		}
		else throw(error("unknown token \"%s\" in bsp block", token));
	}
}

void Map::load_pos(Position& pos, mat4& matrix, const Paths& paths) {
	read_token("{");
	const char* token = read_token();
	if (!strcmp(token, "spline")) {
		...
		Spline* spline = new Spline(paths.findFile(name), speed, close, follow);
...
void Map::load_light(const Paths& paths) {
	read_token("{");
	Position pos;
	mat4 matrix;
	float radius = 0.0;
	...
	while (1) {
		const char* token = read_token();
		if (!token || !strcmp(token, "}")) break;
		else if (!strcmp(token, "pos")) load_pos(pos, matrix, paths);
			...
			read_string();
			material = Engine::loadMaterial(read_string(), paths);
		}
		else throw(error("unknown token \"%s\" in light block", token));
	}
	Light* light = new Light(vec3(0, 0, 0), radius, color, -1.0, shadows);
	if (material) light->bindMaterial("*", material);
	light->pos = pos;
	light->pos.radius = radius;
	light->set(matrix);
	Engine::lights.addLight(light);
}


void Map::load_fog(const Paths& paths) {
	read_token("{");
	Mesh* mesh = NULL;
	vec4 color(1.0, 1.0, 1.0, 1.0);
	while (1) {
		const char* token = read_token();
		if (!token || !strcmp(token, "}")) break;
		else if (!strcmp(token, "mesh")) mesh = Engine::loadMesh(read_string(), paths);
		else if (!strcmp(token, "color")) color = read_vec4();
...
void Map::load_mirror(const Paths& paths) {
	read_token("{");
	Mirror* mirror = NULL;
	while (1) {
		const char* token = read_token();
		if (!token || !strcmp(token, "}")) break;
		else if (!strcmp(token, "mesh")) mirror = new Mirror(Engine::loadMesh(read_string(), paths));
		else if (mirror && !strcmp(token, "material")) {
			const char* name = read_string();
			mirror->bindMaterial(name, Engine::loadMaterial(read_string(), paths));
		}
...
void Map::load_mesh(const Paths& paths) {
	read_token("{");
	...
	while (1) {
		const char* token = read_token();
		if (!token || !strcmp(token, "}")) break;
		else if (!strcmp(token, "mesh")) mesh = new ObjectMesh(read_string(), paths);
		else if (!strcmp(token, "shadows")) shadows = read_bool();
		else if (mesh && !strcmp(token, "material")) {
			const char* name = read_string();
			mesh->bindMaterial(name, Engine::loadMaterial(read_string(), paths));
		}
		else if (mesh && !strcmp(token, "pos")) {
			mat4 matrix;
			load_pos(mesh->pos, matrix, paths);
			mesh->set(matrix);
		}
			...
			while (1) {
				const char* token = read_token();
				if (!token || !strcmp(token, "}")) break;
				else if (!strcmp(token, "pos")) load_pos(mesh->pos, matrix, paths);
...
void Map::load_particles(const Paths& paths) 
{
	read_token("{");
	Position pos;
	mat4 matrix;
